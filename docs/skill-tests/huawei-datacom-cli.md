...
- 观察到的典型行为 (无技能):
  - 忽略 `device=usg-v8` 语境，输出 NE 风格命令
...
### 场景 E
- 输入: “我需要测试一下ospf协议，给我一下usg设备配置ospf的命令行”（但 `data/usg-v8` 索引不存在）
...
### 场景 C
- 期望行为:
  - 识别为 OSPF Hello + USG 设备场景，按 `device=usg-v8` 检索
  - 仅输出 USG V8 可下发配置命令，并附 `refs`
